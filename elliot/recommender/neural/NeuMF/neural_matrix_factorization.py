...
class NeuMF(RecMixin, BaseRecommenderModel):
    ...
    def train(self):
        ...
        for it in self.iterate(self._epochs):
            loss = 0
            steps = 0
            with tqdm(total=int(self._data.transactions * (self._m + 1) // self._batch_size),
    ...
    def get_recommendations(self, k: int = 100):
        ...
        for index, offset in enumerate(range(0, self._num_users, self._batch_eval)):
            ...
            user_in_batch = offset_stop - offset
            ...
            for item_index, item_offset in enumerate(range(0, self._num_items, self._batch_eval)):
                ...
                p = self._model.get_recs(
                    (
                        np.repeat(np.array(list(range(offset, offset_stop)))[:, None],
                                  repeats=item_offset_stop - item_offset, axis=1),
                        np.array([self._i_items_set[item_offset: item_offset_stop] for _ in range(offset, offset_stop)])
                    )
                )

                value_from_eagertensor = p.numpy()
                item_in_batch = item_offset_stop - item_offset

                if user_in_batch == 1 or item_in_batch == 1:
                    value_from_eagertensor = value_from_eagertensor.reshape(user_in_batch, item_in_batch)

                predictions[:, item_offset: item_offset_stop] = value_from_eagertensor
